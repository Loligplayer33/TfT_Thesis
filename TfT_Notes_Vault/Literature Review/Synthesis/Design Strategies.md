...
Fan provides a candidate solution through his finding, that externalizing evaluation criteria for the task can help the user to better understand what he should do and how to approach the task itself. This was the only strategy that lead to increased evaluation processes during revision.
...
- **Externalized evaluation criteria as the activating mechanism for self-evaluation.** The Checklist condition (rubric-based feedback on basics, academic style, originality, integration) was the only support condition that significantly increased Evaluation processes during revision; neither ChatGPT nor an experienced human expert produced this effect. The activating mechanism is therefore plausibly externalized evaluation criteria rather than the presence of a thoughtful interlocutor. Caveat: the task had a rubric, so generalization beyond rubric-bearing tasks is open. (§4.2.1, Figure 3, p14; §4.2.2 contrast with HE (Human Expert), p15)

### [[kreijkesEffectsLLMUse2025]]
- **Suggests sequential separation between note-taking and AI use:** read text → query LLM (clarify/explore) → then take notes independently _without copy-paste capability_. Colombatto with the near-verbatim copying finding enforces the effectiveness of this design strategy [Page 14](zotero://open-pdf/library/items/EE4UFA92?page=14&annotation=ID7YS9E5)
	=> This directly support my idea of hiding the llm conversation whenever text is being written.
